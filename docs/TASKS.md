---
title: 여행 플래너 TASKS
version: v1.0
status: active
created: 2026-03-11
updated: 2026-04-07
owner: daehoonkim
---

## Changelog

| 버전 | 날짜  | 작성자     | 변경 내용                             |
| ---- | ----- | ---------- | ------------------------------------- |
| v1.0 | 03-11 | daehoonkim | 초안 작성 — 현재 코드베이스 기반 역추적 |

---

# TASKS: Travel Planner

> **최종 수정일**: 2026-04-18
> **관련 문서**: [PRD](./PRD.md) | [TRD](./TRD.md) | [CHECKLIST](./DEVELOPMENT-CHECKLIST.md)

---

## 범례

| 상태 | 의미                  |
| ---- | --------------------- |
| 🔴   | Blocked (차단됨)      |
| 🟡   | In Progress (진행 중) |
| 🟢   | Ready (착수 가능)     |
| ✅   | Completed (완료)      |
| ⏸️   | On Hold (보류)        |

| 우선순위 | 의미                       |
| -------- | -------------------------- |
| P0       | 필수 (출시 블로커)         |
| P1       | 중요 (출시 직후)           |
| P2       | 개선 (여유 있을 때)        |
| P3       | Nice-to-have               |

---

## 🟡 현재 진행 중

| 작업 | 상태 | 설명 | 관련 문서 |
| ---- | ---- | ---- | --------- |
| (없음) | - | - | - |

> **최종 수정일**: 2026-04-18

---

## 🟢 백로그

### P0 - 출시 핵심

| 작업                           | 상태 | 설명                                                  | 관련 문서 |
| ------------------------------ | ---- | ----------------------------------------------------- | --------- |
| ~~여행 삭제 기능~~             | ✅   | 완료 (2026-04-06)                                      | -         |
| ~~장소 수정 기능~~             | ✅   | 완료 (2026-03-19)                                      | -         |
| **에러 핸들링 통일**           | 🟢   | 공통 에러 바운더리 + API 응답 형식 `{ success, data, error }` | -         |
| **일정 시간 입력 (start_time)**| 🟢   | schedule_item에 시작 시간 UI 추가 → Routes API departureTime 연동 | -         |
| **OAuth 클라이언트 재생성**    | 🟢   | Google 로그인용 OAuth 클라이언트 삭제됨 → 재생성 + Supabase 연동 필요 | -         |

### P1 - CRUD 보완

| 작업                           | 상태 | 설명                                                  | 관련 문서 |
| ------------------------------ | ---- | ----------------------------------------------------- | --------- |
| ~~멤버 역할 변경~~             | ✅   | 완료 (2026-03-19)                                      | -         |
| ~~멤버 제거~~                  | ✅   | 완료 (2026-03-19)                                      | -         |
| 장소 카테고리 필터             | 🟢   | 탭 또는 드롭다운 UI                                    | -         |
| AI 결과 → 일정 직접 반영       | 🟢   | AI 응답 JSON 파싱 → schedule_items 자동 삽입           | -         |

### P1 - 개발 환경

| 작업                           | 상태 | 설명                                                  | 관련 문서 |
| ------------------------------ | ---- | ----------------------------------------------------- | --------- |
| ESLint + Prettier 설정         | 🟢   | 코드 스타일 통일                                       | -         |
| Git 브랜칭 전략 수립           | 🟢   | trunk-based 또는 feature branch                        | -         |
| 커밋 메시지 컨벤션             | 🟢   | Conventional Commits (feat/fix/docs)                   | -         |
| CD: Vercel 자동 배포           | 🟢   | main → production, PR → preview                       | -         |

### P1 - 테스트

| 작업                           | 상태 | 설명                                                  | 관련 문서 |
| ------------------------------ | ---- | ----------------------------------------------------- | --------- |
| ~~Integration: /api/places/share~~ | ✅   | 완료 (2026-04-06) — 인증/Rate limit/중복/성공 11개 케이스 | -         |

### P1 - 모니터링

| 작업                           | 상태 | 설명                                                  | 관련 문서 |
| ------------------------------ | ---- | ----------------------------------------------------- | --------- |
| 에러 트래킹 도구 연결 없음     | ⚪   | 외부 에러 트래킹 도구는 현재 사용하지 않음            | -         |
| Vercel Analytics 연결          | 🟢   | Web Vitals 추적                                        | -         |
| Google API 사용량 모니터링     | 🟢   | Maps/Places/Gemini 무료 티어 한도 알림                 | -         |

### P2 - 디자인

| 작업                           | 상태 | 설명                                                  | 관련 문서 |
| ------------------------------ | ---- | ----------------------------------------------------- | --------- |
| 컬러 팔레트 정의               | 🟢   | Primary, Secondary, Neutral, Semantic 색상             | -         |
| 타이포그래피 스케일             | 🟢   | 제목/본문/캡션 크기, 행간 규칙                         | -         |
| 다크모드 컬러 토큰             | 🟢   | next-themes 사용 중, 토큰 미정의                       | -         |
| 와이어프레임 (Figma)           | 🟢   | 주요 10개 화면 Lo-fi → Hi-fi                           | -         |

### P2 - 접근성 (WCAG 2.1 AA)

| 작업                           | 상태 | 설명                                                  | 관련 문서 |
| ------------------------------ | ---- | ----------------------------------------------------- | --------- |
| 폼 접근성 강화 (Task A)        | 🟢   | aria-describedby, aria-required, 에러 시 포커스 이동   | DEC-93 |
| 아이콘/데코레이션 접근성 (Task B) | 🟢 | 장식 아이콘 aria-hidden, 아이콘 버튼 sr-only 레이블   | DEC-93 |
| hover-only → focus 포함 (Task C) | 🟢 | draggable-item 액션 버튼 키보드/터치 접근성           | DEC-93 |
| 터치 타겟 최소 44px (Task D)   | 🟢   | vote-button, checklist 체크 버튼 히트 영역 확대       | DEC-93 |
| aria-live 실시간 공지 (Task E) | 🟢   | 토스트/로딩/에러 스크린 리더 공지                     | DEC-93 |

### P2 - 개선

| 작업                           | 상태 | 설명                                                  | 관련 문서 |
| ------------------------------ | ---- | ----------------------------------------------------- | --------- |
| 지출 차트 시각화               | 🟢   | recharts 활용 카테고리별 파이차트/바차트               | -         |
| 여행 커버 이미지 업로드        | 🟢   | Supabase Storage 연동                                  | -         |
| ~~OG 메타태그~~                | ✅   | 완료 (2026-04-14) — Twitter Card 포함                  | -         |
| ~~랜딩 페이지~~                | ✅   | 완료 (2026-04-14) — Magic Link 로그인 전환 포함        | -         |
| 이용약관 & 개인정보처리방침    | 🟢   | 법적 필수 문서                                         | -         |

### P3 - Nice-to-have

| 작업                           | 상태 | 설명                                                  | 관련 문서 |
| ------------------------------ | ---- | ----------------------------------------------------- | --------- |
| 환율 자동 변환                 | 🟢   | 실시간 환율 API 연동                                   | -         |
| 후기 외부 공유 링크            | 🟢   | 비회원도 볼 수 있는 공개 링크                          | -         |
| 푸시 알림 (Web Push)           | 🟢   | 멤버 활동 알림                                         | -         |
| ~~E2E 테스트 (Playwright)~~    | ✅   | 완료 (2026-04-14) — fixture 계정 + auth/travel 플로우 | -         |

---

## ✅ 완료됨

### 2026-04 (04-10 ~ 04-18)

| 작업                              | 완료일     | 설명                                                        |
| --------------------------------- | ---------- | ----------------------------------------------------------- |
| **랜딩 페이지** (N-16)            | 2026-04-14 | 서비스 소개 + Magic Link 로그인 전환 + dev 빠른 로그인 (commit 14d0ee6) |
| **OG 메타태그** (N-09)            | 2026-04-14 | 소셜 공유 미리보기 + Twitter Card (commit 346003a) |
| **E2E 테스트 (Playwright)**        | 2026-04-14 | fixture 계정 안정화, auth/travel-flows/quality-gates 스펙 (commit 8521e62) |
| **스케줄 Haversine 이동 정보**     | 2026-04-14 | 장소 이동 사이 거리/시간 자동 표시 (commit aacc894) |
| **버그 수정 배치**                 | 2026-04-14 | 체크리스트 토글, 프로필 Invalid Date, Undo 에러처리, 알림 SSRF (commit 9fc82a2) |

### 2026-04 (04-01 ~ 04-09)

| 작업                              | 완료일     | 설명                                                        |
| --------------------------------- | ---------- | ----------------------------------------------------------- |
| **Phase 11: Advanced Optimization** | 2026-04-09 | 11-1 Server Component prefetch, 11-2 외부 API 재시도, 11-3 CSP nonce 강화, 11-4 RealtimeProvider 분할 (commit 7e70b2d) |
| **Quality Fix: Phase 8 (Realtime 동기화)** | 2026-04-07 | place_votes trip_id 컬럼+필터, schedule_items Realtime 필터, Query Key Factory 15개 파일, MembersPage React Query 전환 |
| **Schedule Planner (Phase 4-6)** | 2026-04-07 | Push Notification(VAPID+SW+Edge Fn), Geolocation GPS 보강, departure-alert 컴포넌트 |
| **Guards Coverage 테스트** | 2026-04-07 | API route 49개 가드 적용 검증 테스트 (withAuth/withTripMember/withTripEditor) |
| **Quality Fix: Phase 9 (Components)** | 2026-04-06 | PlaceImage lazy loading, 사진 병렬화(Promise.allSettled CONCURRENCY=5), Error boundary 4개, SW 이미지 캐시, console.log 정리 |
| **외부 API 재시도 로직 (11-2)** | 2026-04-06 | fetchWithRetry 유틸 추가, Quality Fix |
| **일정 추가 UI 즉시 반영** | 2026-04-06 | 이동거리 계산 비동기 분리 (perf) |
| **여행 삭제 기능** | 2026-04-06 | DELETE /api/trips/[tripId] (admin 전용) + 5초 Undo 패턴, 낙관적 업데이트 |
| **Integration: /api/places/share** | 2026-04-06 | 인증/Rate limit/중복 감지/성공 케이스 11개 (Vitest 모킹) |
| **Quality Fix: Phase 5 (travel-info-card)** | 2026-04-01 | markers 인코딩 수정, polyline 길이 상수화, duration NaN 방어, 지도 로딩 에러 UI |
| **Quality Fix: Phase 6 (DB 기반)** | 2026-04-01 | DB 인덱스 6개, reorder_schedule_items RPC, activity_logs 불변화, place_votes UPDATE RLS, notifications uuid 타입, get_contribution_stats RPC |
| **Quality Fix: Phase 7 (API 보안)** | 2026-04-01 | enrich viewer 차단, Place ID 검증, share FormData 검증, ISO 8601 cursor 검증, 에러 응답 통일, Gemini key trim |
| **Quality Fix: Phase 10 (인프라)** | 2026-04-01 | ESLint CRITICAL 6건, pnpm audit fix, Next.js 16.2.2, middleware→proxy 리네임, PWA 아이콘, MemberContext tripId |

### 2026-03 (03-01 ~ 03-20)

| 작업                              | 완료일     | 설명                                                        |
| --------------------------------- | ---------- | ----------------------------------------------------------- |
| **AppShell 성능 최적화** | 2026-03-20 | getUser→getSession, loading gate 제거, 폰트 preconnect |
| **활동 자동 기록 (activity_logs)** | 2026-03-19 | DB 트리거로 places/schedule/checklist CUD 자동 로깅 |
| **알림 시스템 (notifications)** | 2026-03-19 | activity_logs → 멤버별 알림 자동 생성, 알림 페이지/배지 |
| **게스트 모드 (초대 미리보기)** | 2026-03-19 | 비회원 초대 링크 → 여행 정보 미리보기 후 가입 유도 |
| **장소 수정 기능** | 2026-03-19 | place-form 재활용, 수정 모드 |
| **멤버 역할 변경/제거** | 2026-03-19 | admin 전용 역할 변경 + 멤버 제거 UI |
| **PlaceCard 이미지 Next/Image 전환** | 2026-03-18 | Next/Image 적용, Card plain variant 추가 |
| **체크리스트 인라인 빠른 추가** | 2026-03-18 | 카테고리별 '+' 버튼, 글로벌 페이지 추가 지원 |
| **체크리스트 액션 DropdownMenu 통합** | 2026-03-17 | 모바일 가독성 개선, 제목 가시 영역 60% 증가 |
| **모바일 장소 추가 UX 개선** | 2026-03-17 | Tap-to-Assign DayPickerSheet + UnscheduledFAB |
| **글로벌 체크리스트 페이지** | 2026-03-17 | BottomNav에 체크리스트 탭 배치 |
| **모바일 장소 카드 레이아웃 변경** | 2026-03-16 | 컴팩트 수평 레이아웃 |
| **경로 단계 클릭 → 지도 확대** | 2026-03-16 | StepList-MiniMap 인터랙션 |
| **이동 경로 상세 표시** | 2026-03-16 | 노선명/정거장/도보 구간 StepList, 폴리라인 |
| **체크리스트 RPC + DB 트리거** | 2026-03-15 | Phase 2 — 보안/원자성 강화 |
| **UX 검토 수정** | 2026-03-15 | 여행 생성 에러, 일정 에러 UI, 날짜 형식 통일 |
| **예산/후기 기능 제거** | 2026-03-14 | 핵심 기능(장소/일정/체크리스트/멤버)만 유지 |
| **Undo 삭제 패턴 도입** | 2026-03-14 | confirm() 제거, 5초 되돌리기 토스트 |
| **참여자 온보딩 강화** | 2026-03-14 | 참여 후 웰컴 토스트 + 초대 페이지 미리보기 |
| **여행 진행 배너** | 2026-03-14 | 장소/일정 완성도 한눈에 확인 |
| **Directions Gateway + AI 보안** | 2026-03-14 | 경로 API 분리 + 프롬프트 인젝션 방어 (111 tests) |
| **CSP 강화 + 타입 자동 생성** | 2026-03-13 | unsafe-eval 제거, Supabase 타입 자동 생성 스크립트 |
| **초대 코드 보안 강화** | 2026-03-13 | 최대 멤버 수 제한(20명), admin 코드 재생성 |
| **Schedule React Query 전환** | 2026-03-13 | Realtime invalidation 자동 연결 |
| **Schedule 페이지 분해** | 2026-03-13 | useScheduleData/useScheduleActions 훅 추출 (646→310줄) |
| **GitHub Actions CI 파이프라인** | 2026-03-12 | 빌드+린트+타입체크 + 98 tests |
| **전 API Route withAuth 가드** | 2026-03-12 | 가드 적용 검증 테스트 27개 |
| **photo 프록시 인증 + weather/directions 보안** | 2026-03-12 | withAuth 전환 |
| **순수 함수 테스트 추가** | 2026-03-12 | resolveInput, haversine, duration, weatherMeta (31→71 tests) |
| **기획 문서 체계 구축** | 2026-03-11 | PRD/TRD/TASKS/CHECKLIST 작성 |
| **Google Places API 연동**        | 2026-03-11 | Text Search + Place Details + Photo URL + 비동기 풍부화     |
| **PWA Share Target**              | 2026-03-11 | 모바일 공유 → 자동 저장 + Sticky Context (30분)             |
| **반응형 네비게이션**             | 2026-03-10 | 데스크톱 사이드바(64px) + 모바일 BottomNav                  |
| **여행 정보 수정**                | 2026-03-09 | 제목/목적지/날짜 수정 다이얼로그                            |
| **장소 상세 페이지**              | 2026-03-09 | `/trips/[tripId]/places/[placeId]` 상세 뷰                  |
| AI 플로팅 버튼 위치 수정          | 2026-03-08 | z-index 수정, BottomNav 위로 표시                           |
| DnD 중첩 컨텍스트 제거            | 2026-03-08 | 장소 드래그앤드롭 정상 동작                                 |
| **AI 채팅 플로팅 버튼 전환**      | 2026-03-07 | 별도 페이지 → FAB + 바텀시트 팝업                           |
| 장소 카드 삭제 버튼               | 2026-03-07 | 장소 목록에서 직접 삭제                                     |
| AI 채팅 Gemini 연결               | 2026-03-06 | 프론트엔드 → 실제 Gemini API 엔드포인트 연결                |
| Google Places 상세 정보 추가      | 2026-03-06 | 검색 결과에서 주소/평점/사진 가져오기                       |
| PlaceSearch React 재구현          | 2026-03-05 | AutocompleteService + React UI로 전환                       |
| **URL 스크래핑 기능**             | 2026-03-04 | Booking/Agoda 등 12개 사이트 메타데이터 추출 + SSRF 방어    |
| PlaceSearch DOM 충돌 해결         | 2026-03-04 | React DOM 호환성 수정                                       |
| PlaceSearch API 전환              | 2026-03-03 | 새 PlaceAutocompleteElement API 적용                        |
| **장소 등록 시 Google 검색**      | 2026-03-02 | 지도 검색 → 자동 좌표/주소 채움                             |
| **MVP 전체 구현**                 | 2026-03-01 | 여행 CRUD, 장소, 일정, 예산, 후기, 멤버, Realtime, PWA      |

### 이전

| 작업                              | 설명                                                        |
| --------------------------------- | ----------------------------------------------------------- |
| 프로젝트 초기화                   | Create Next App + TypeScript + Tailwind                     |
| Supabase 프로젝트 연결            | Auth, DB, Realtime 설정                                     |

---

## ⏸️ 보류/검토 필요

| 작업                        | 상태 | 사유                                                 |
| --------------------------- | ---- | ---------------------------------------------------- |
| -                           | -    | -                                                    |

---

## 기술 부채

| 항목                       | 우선순위 | 설명                                                          |
| -------------------------- | -------- | ------------------------------------------------------------- |
| Rate Limiting In-memory    | P1       | 서버리스에서 리셋됨 → Redis 또는 Supabase 기반 전환 필요       |
| 이미지 최적화              | P2       | 외부 URL 직접 렌더링 → Next/Image + Supabase Storage 프록시   |
| 번들 분석 미수행            | P2       | `@next/bundle-analyzer` 도입 필요                             |
| 환경 분리                   | P2       | .env.local 단일 → 개발/스테이징/프로덕션 분리                 |
| 예산/후기 기능 복원 검토   | P2       | 제거된 기능의 재도입 시점/범위 판단 필요                       |
| console.log 잔존           | P3       | PlaceForm 4건 제거 완료 (2026-04-06) — 다른 파일 잔존 여부 모니터링 필요 |

---

## 아이디어/제안

> 향후 검토할 아이디어를 기록합니다.

| 아이디어                  | 설명                                    | 관련 문서 |
| ------------------------- | --------------------------------------- | --------- |
| 여행 템플릿               | "도쿄 3박4일" 등 인기 여행 템플릿 제공  | -         |
| 여행 공개/탐색            | 다른 사용자의 여행 계획 탐색            | -         |
| 환율 계산기               | 앱 내 실시간 환율 변환 도구             | -         |
| 항공편/교통편 연동        | 항공편 검색 및 일정 자동 추가           | -         |

---

## 변경 이력

| 날짜       | 변경 내용                                                          |
| ---------- | ------------------------------------------------------------------ |
| 2026-04-18 | 04-10~18 완료 항목 추가 (랜딩/OG/E2E/Haversine/버그수정), 백로그 완료 항목 업데이트 |
| 2026-04-07 | Quality Fix Phase 5~10 완료 항목 추가, Guards Coverage 테스트 추가, Phase 11 진행 중 표시 |
| 2026-03-18 | 완료 항목 24개 추가, 백로그에서 완료된 항목 제거, 기술부채 업데이트 |
| 2026-03-11 | 초안 작성 — git log 기반으로 완료 항목 역추적, 백로그 우선순위 정리 |
