...
Phase 9: Components & Performance — Status: in_progress
...
- [ ] 8-4. **place_votes Realtime 필터** — 별도 (place_votes에 trip_id 컬럼 없음, 스키마 검토 필요)
- [ ] 8-5. **schedule_items Realtime 필터** — 별도 (schedule_id 기반 필터 적용 검토)
- [x] 8-6. **MembersPage** — useState/useEffect → useTripMembers hook + React Query optimistic
- **Status:** 4/6 done (8-4, 8-5 스키마 검토 필요) — 2026-04-01
...
- [ ] 11-2. 외부 API 1회 재시도 + exponential backoff
